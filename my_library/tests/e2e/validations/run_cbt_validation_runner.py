...
for both classification and regression tasks with CustomCatBoost.
Demonstrates runner methods beyond calibrate_proba and handles report output directory.
"""
...
OUTPUT_DIR = "my_library/output/cv_reports"
os.makedirs(OUTPUT_DIR, exist_ok=True)


def run_classification_cv():
    """
    Perform 3-fold CV on the Iris classification dataset,
    and demonstrate additional ValidationRunner methods.
    """
    print("\n--- Classification: Iris 3-Fold CV ---")
    ...
    # Ensemble predictions (average proba or vote)
    ...
    # Export CV report to output directory
    runner.export_cv_report(
        os.path.join(OUTPUT_DIR, "cv_report_classification.csv"), as_excel=False
        )
    runner.export_cv_report(
        os.path.join(OUTPUT_DIR, "cv_report_classification.xlsx"), as_excel=True
        )
    print("Exported CV reports for classification to", OUTPUT_DIR)

    # Retrain on all training data using best iteration * TEST_ROUND_MULTIPLIER
...
def run_regression_cv():
    """
    Perform 3-fold CV on the Diabetes regression dataset,
    and demonstrate additional ValidationRunner methods.
    """
    print("\n--- Regression: Diabetes 3-Fold CV ---")
    ...
    reg_config = CatBoostConfig(
        model_name="CatBoost_Diabetes_CV",
    ...
    # Export CV report to output directory
    runner.export_cv_report(os.path.join(OUTPUT_DIR, "cv_report_regression.csv"), as_excel=False)
    runner.export_cv_report(os.path.join(OUTPUT_DIR, "cv_report_regression.xlsx"), as_excel=True)
