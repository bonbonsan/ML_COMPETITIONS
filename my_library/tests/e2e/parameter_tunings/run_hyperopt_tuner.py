...
from my_library.parameter_tunings.hyperopt_tuner import HyperoptValidationTuner
...
param_space = {
    "depth": (4, 8),        # will be sampled uniformly between 4 and 8
    "iterations": (50, 200) # will be sampled uniformly between 50 and 200
}
tuner_clf = HyperoptValidationTuner(
...
# If you want full runner results as well:
# best_params_clf, best_score_clf, runner_res_clf = \
#   tuner_clf.tune(fit_cfg, return_runner_results=True)
...
tuner_reg = HyperoptValidationTuner(
    model_class=CustomCatBoost,
    model_configs=reg_config,
    param_space=param_space,  # reuse same param_space
    folds=reg_folds,
