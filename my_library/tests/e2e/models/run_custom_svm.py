...
# configure CustomSVM for classification
clf_config = SVMConfig(
    model_name="CustomSVM_Iris",
    ...
    params={
        ...
        "C": 1.0,
        "gamma": "scale",
        # max_iter=-1 means no limit (useful for large datasets)
        "max_iter": -1
    }
)
...
clf_model = CustomSVM(config=clf_config)
...
# configure CustomSVM for regression
reg_config = SVMConfig(
    model_name="CustomSVM_Diabetes",
...
reg_model = CustomSVM(config=reg_config)
