...
# pytest my_library/tests/unit/parameter_tunings/test_hyperopt_tuner.py -v
...
def test_tune_returns_best_params_and_score(tuner_and_data):
    ...
    # Internal state should match return
    assert tuner.best_params == best_params
    assert tuner.best_score == best_score

    # Score sign: compare against per-fold scores
...
def test_tune_with_runner_results(tuner_and_data):
    ...
    # Ensure the tuner's internal runner_result matches
