...
# pytest my_library/tests/unit/parameter_tunings/test_random_tuner.py -v
...
def tuner_and_data(request):
    ...
    tuner = RandomSearchValidationTuner(
...
def test_tune_returns_best_params_and_score(tuner_and_data):
    ...
    # Score sign
    if tuner.maximize:
        assert best_score >= min(tuner.best_runner_result['fold_scores'])
    else:
        assert best_score <= max(tuner.best_runner_result['fold_scores'])
