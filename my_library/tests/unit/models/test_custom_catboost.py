...
# pytest my_library/tests/unit/models/test_custom_catboost.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression", "diabetes", "target")
])
def test_catboost_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load data
    ...
    path = tmp_path / f"model_{dataset_name}_{task_type}_cat.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomCatBoost(config=config)
