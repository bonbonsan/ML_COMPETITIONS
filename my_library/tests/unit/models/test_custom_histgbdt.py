...
# pytest my_library/tests/unit/models/test_custom_histgbdt.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression", "diabetes", "target")
])
def test_histgbdt_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load data
    ...
    # Prepare config
    config = HistGBDTConfig(task_type=task_type, save_log=False)
    ...
    model = CustomHistGBDT(config=config)
    ...
    path = tmp_path / f"model_{dataset_name}_{task_type}_hist.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomHistGBDT(config=config)
