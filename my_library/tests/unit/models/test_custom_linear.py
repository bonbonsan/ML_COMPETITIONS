...
# pytest my_library/tests/unit/models/test_custom_linear.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression",    "diabetes",   "target"),
])
def test_custom_linear_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load sample data
    ...
    config = LinearConfig(task_type=task_type, save_log=False)
    model = CustomLinear(config=config)
    ...
    # ---- Test save/load ----
    path = tmp_path / f"custom_linear_{dataset_name}_{task_type}.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomLinear(config=config)
