...
# pytest my_library/tests/unit/models/test_custom_lightgbm.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression", "diabetes", "target")
])
def test_lightgbm_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load data
    ...
    # Prepare config
    config = LightGBMConfig(task_type=task_type, use_gpu=False, save_log=False)
    model = CustomLightGBM(config=config)
    ...
    path = tmp_path / f"model_{dataset_name}_{task_type}.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomLightGBM(config=config)
