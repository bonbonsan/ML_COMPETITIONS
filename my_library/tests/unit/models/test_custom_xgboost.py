...
# pytest my_library/tests/unit/models/test_custom_xgboost.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression", "diabetes", "target")
])
def test_xgboost_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load data
    ...
    # Prepare config
    config = XGBoostConfig(task_type=task_type, use_gpu=False, save_log=False)
    model = CustomXGBoost(config=config)
    ...
    # Ensure regression raises NotImplementedError
    else:
    ...
    # Save and load using temporary path
    path = tmp_path / f"model_{dataset_name}_{task_type}_xgb.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomXGBoost(config=config)
