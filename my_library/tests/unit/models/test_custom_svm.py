...
# pytest my_library/tests/unit/models/test_custom_svm.py -v
@pytest.mark.parametrize("task_type,dataset_name,target_column", [
    ("classification", "iris", "target"),
    ("regression",    "diabetes",   "target"),
])
def test_custom_svm_model(task_type, dataset_name, target_column, tmp_path: Path):
    # Load sample data
    ...
    config = SVMConfig(task_type=task_type, save_log=False)
    model = CustomSVM(config=config)
    ...
    # ---- Test save/load ----
    path = tmp_path / f"custom_svm_{dataset_name}_{task_type}.pkl"
    model.save_model(str(path))
    assert path.exists()

    new_model = CustomSVM(config=config)
